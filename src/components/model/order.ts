...
class OrderModel {
  checkField(fieldName: string, fieldValue: string): checkResult {
    const validation: [string, (value: string) => boolean, string][] = [
      [
        'order-name',
        (value: string): boolean => value.split(' ').filter((_) => _.length >= 3).length >= 2,
        'Field must contain at least 2 words with more than 3 letters',
      ],
      [
        'order-phone',
        (value: string): boolean => this.checkByRegExp(value, '^\\+\\d{9,12}'),
        'Phone must start with + and contain from 9 to 12 digits',
      ],
      [
        'order-address',
        (value: string): boolean => value.split(' ').filter((_) => _.length >= 5).length >= 3,
        'Adress must contain at least 3 words with more than 5 letters',
      ],
      [
        'order-email',
        (value: string): boolean => this.checkByRegExp(value, '^\\w+([\\.-]?\\w+)*@\\w+([\\.-]?\\w+)*(\\.\\w{2,3})+$'),
        'Not valid email',
      ],
      [
        'order-card-number',
        (value: string): boolean => value !== '' && this.isCreditCardNumberValid(value),
        'Invalid card number',
      ],
      ['order-card-until', (value: string): boolean => this.isCreditCardUntilValid(value), 'Invalid date'],
      [
        'order-card-cvv',
        (value: string): boolean => this.checkByRegExp(value, '^\\d\\d\\d$'),
        'CVV code must contain 3 digits',
      ],
    ];
